...
export function CompletionTab({ responses, stats }: CompletionTabProps) {
  ...
  useEffect(() => {
    const fetchAcceptedInvitations = async () => {
      try {
        const response = await axios.get(`https://swbackstg.onrender.com/survey/invitationsurvey/${id}`);
  ...
  const handleRetarget = async (email: string, surveyID: string, progress: number) => {
    ...
    try {
      await axios.post("https://swbackstg.onrender.com/retarget_survey", { email, surveyID, progress });
