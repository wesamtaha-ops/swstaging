...
export function CompletionTab({ responses, stats }: CompletionTabProps) {
  ...
  useEffect(() => {
    const fetchAcceptedInvitations = async () => {
      try {
        const response = await axios.get(`https://backend.votly.app/survey/invitationsurvey/${id}`);
  ...
  const handleRetarget = async (email: string, surveyID: string, progress: number) => {
    ...
    try {
      await axios.post("https://backend.votly.app/retarget_survey", { email, surveyID, progress });
