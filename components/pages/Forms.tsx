...
export default function Forms() {
  ...
  const fetchSurveys = async () => {
    try {
      ...
      //   apiUrl = `https://swbackstg.onrender.com/getsurveyByUserId/${userId}`;
      // } else {
      apiUrl = `https://swbackstg.onrender.com/user/survey/${workspaceId}`;
      // }
      ...
      const completionPromises = data.map(async (survey: Form) => {
        try {
          const res = await axios.get(
            `https://swbackstg.onrender.com/surveys/completion/${survey._id}`
          );
          return {
            id: survey._id,
      ...
      const avgTimesPromises = data.map(async (survey: Form) => {
        try {
          const res = await axios.get(
            `https://swbackstg.onrender.com/survey/average_time/${survey._id}`
          );
  ...
  useEffect(() => {
    const fetchResponsesCount = async () => {
      ...
      for (const survey of currentSurveys) {
        try {
          const res = await axios.get(
            `https://swbackstg.onrender.com/submit_survey/responses/${survey._id} `
          );
