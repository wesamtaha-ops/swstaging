...
export function AccessPanel({ isClosed, onChangeIsClosed }: AccessPanelProps) {
  ...
  const fetchAccessSettings = async () => {
    try {
      const res = await fetch(`http://localhost:5050/getsurvey/${surveyId}`);
  ...
  const updateSurveyAccess = async () => {
    try {
      ...
      await fetch(`http://localhost:5050/surveys/access/${surveyId}`, {
        method: "PUT",
  ...
  const handleCloseForm = async () => {
    try {
      const res = await fetch(`http://localhost:5050/close_survey/${surveyId}/close`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      });
  ...
  const handleOpenForm = async () => {
    try {
      const res = await fetch(`http://localhost:5050/close_survey/${surveyId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      });
